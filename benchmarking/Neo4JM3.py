...
class Neo4jPC(M3DB):
    ...
    def __get_tags_in_tagset(tx, tagset_id):
        result = tx.run("MATCH (t:Tag)-[:IN_TAGSET]->(ts:Tagset {id: $tagset_id}) RETURN t.id, labels(t)",
                        tagset_id=tagset_id)
        return list(result)
    ...
    def __get_level_from_parent_node(tx, node_id, hierarchy_id):
        result = tx.run(
            "MATCH (root:Node {id: $node_id})<-[:HAS_PARENT]-(n : Node)-[:IN_HIERARCHY]->(h:Hierarchy {id: $hierarchy_id}) "
    ...
    def get_objects_with_tag(self, tag_id):
        with self.driver.session() as session:
            return session.read_transaction(self.__get_objects_with_tag, tag_id)

    @staticmethod
    def __get_objects_with_tag(tx, tag_id):
        result = tx.run("MATCH (t:Tag {id: $tag_id})<-[:TAGGED]-(o: Object) RETURN o.id, o.file_uri", tag_id=tag_id)
        return list(result)

    @staticmethod
    def __insert_object(tx, id, file_uri, file_type, thumbnail_uri):
        tx.run(
            "CREATE (o:Object:Benchmark {id: $id, file_uri: $file_uri, file_type: $file_type, thumbnail_uri: $thumbnail_uri})",
    ...
    @staticmethod
    def __get_objects_with_tag(tx, tag_id):
        result = tx.run("MATCH (t:Tag {id: $tag_id})<-[:TAGGED]-(o: Object) RETURN o.id", tag_id=tag_id)
        return list(result)
    ...
    @staticmethod
    def __get_objects_in_tagset(tx, tagset_id):
        result = tx.run(
            "MATCH (ts:Tagset {id: $tagset_id})<-[:IN_TAGSET]-(t:Tag)<-[:TAGGED]-(o:Object) RETURN o.id, ts.id",
            tagset_id=tagset_id)
        return list(result)

    def get_objects_in_tagset(self, tagset_id):
        with self.driver.session() as session:
            return session.read_transaction(self.__get_objects_in_tagset, tagset_id)

    @staticmethod
    def __get_objects_in_subtree(tx, node_id):
        result = tx.run("MATCH (dim1_n: Node {id: $node_id}) "
                        "MATCH (dim1_n)<-[:HAS_PARENT]-(R1 : Node)<-[:HAS_PARENT*0..]-(: Node)-[:REPRESENTS]->(t:Tag)<-[:TAGGED]-(o: Object) "
                        "RETURN o.id, t.id", node_id=node_id)
        return list(result)

    def get_objects_in_subtree(self, node_id):
        with self.driver.session() as session:
            return session.read_transaction(self.__get_objects_in_subtree, node_id)
