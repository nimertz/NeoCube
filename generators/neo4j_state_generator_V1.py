...
def get_object_in_node_subtree(tx, node):
    object_count_query = "MATCH (root: Node {id: %i })<-[:HAS_PARENT*0..]-()-[:REPRESENTS]->()<-[:TAGGED]-(o:Object) RETURN count(distinct o) as cnt;" % (
        node)
...
def get_tags_in_tagset(tx, tagset_id):
    result = tx.run("MATCH (t:Tag)-[r:IN_TAGSET]->(ts:Tagset {id: $tagset_id}) RETURN t.id,t.name, labels(t), ts.id",
                    tagset_id=tagset_id)
    return list(result)
...
with driver.session() as session:
    for L in sys.stdin:
        ...
        if C[0] == "S":
            ...
            objects_query = "MATCH (o:Object)-[:TAGGED]->(t:Tag) WHERE t.id IN %s RETURN count(distinct o) as cnt;" % (
                tagset_tags)
